...
def _test_feature_extractors(
    ...
    for name, builder in extractors.items():
        ...
        featuresRoi,x=fe.featureROI([input], [box_list] * N)
        # self.assertEqual(
        #     out.shape[:2],
...
class TestFeatureExtractors(unittest.TestCase):
    ...
    #         FEATURE_EXTRACTORS_INPUT_CHANNELS,
    #     )
    #TODO  tow registry (registry.ROI_KEYPOINT_FEATURE_EXTRACTORS registry.ROI_MASK_FEATURE_EXTRACTORS) still not exist.
