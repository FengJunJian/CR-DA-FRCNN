...
def do_train(
    ...
    for iteration, (images, targets, _) in enumerate(data_loader, start_iter + 1):
        ...
        if iteration % 20 == 0 or iteration == max_iter:##################################visual
            eta_seconds = meters.time.global_avg * (max_iter - iteration)
            eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
            for k, v in meters.toDict().items():
                writerT.add_scalar(tag=k, scalar_value=v, global_step=iteration)
                writerT.flush()
            writerT.add_scalar(tag='lr',scalar_value=optimizer.param_groups[0]["lr"],global_step=iteration)
...
def do_da_train(
    ...
    for iteration, ((source_images, source_targets, idx1), (target_images, target_targets, idx2)) in enumerate(zip(source_data_loader, target_data_loader), start_iter+1):
        data_time = time.time() - end
        arguments["iteration"] = iteration
