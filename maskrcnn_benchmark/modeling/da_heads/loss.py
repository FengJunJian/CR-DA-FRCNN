...
class DALossComputation(object):
    ...
    def __call__(self, da_img, da_ins, da_img_consist, da_ins_consist, da_ins_labels, targets):
        ...
        masks = torch.cat(masks, dim=0).bool()#convert to bool type
        ...
        da_img_loss = F.binary_cross_entropy_with_logits(#da_img_loss
            da_img_flattened, da_img_labels_flattened
        )
        da_ins_loss = F.binary_cross_entropy_with_logits(#da_ins_loss
            torch.squeeze(da_ins), da_ins_labels.type(torch.cuda.FloatTensor)
        )

        da_consist_loss = consistency_loss(da_img_consist, da_ins_consist, da_ins_labels, size_average=True)#da_consist_loss
