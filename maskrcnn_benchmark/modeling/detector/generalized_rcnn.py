...
class SW_DA_RCNN(GeneralizedRCNN):
    """
    Main class for Generalized R-CNN. Currently supports boxes and masks.
    It consists of three main parts:
    - backbone
    ...
    def __init__(self, cfg):
        super(GeneralizedRCNN, self).__init__()
        self.cfg=cfg
        backbone = build_backbone(cfg)#feature extractor  backbone/backbone.py
        #a=load_resnet_c2_format(cfg,'../R-50.pkl')
        #model = nn.Sequential(OrderedDict([("body", body)]))

        self.backbone1=nn.Sequential(
            OrderedDict(list(backbone.body.named_children())[:2]),
        )
        self.backbone2=nn.Sequential(OrderedDict(list(backbone.body.named_children())[2:]))
    ...
    def forward(self, images, targets=None):
        ...
        # backbone.to(self.cfg.MODEL.DEVICE)
        if self.training and targets is None:#model forward
            raise ValueError("In training mode, targets should be passed")
        images = to_image_list(images)#[1,3,608,1088]
        #features = backbone(images.tensors)#features [[2,1024,38,68],]
